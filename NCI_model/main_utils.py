...
def load_data(args):
    ...
    if 'gtq' in args.query_type:
        ...
        if 'qg' in args.query_type:
            if args.trivia:
                qg_file = join(args.data_dir, 'trivia_dataset/trivia_512_qg.tsv')
                gq_df1 = pd.read_csv(
                    qg_file,
    ...
    if 'doc' in args.query_type:
        if args.trivia:
            filename = join(args.data_dir, 'trivia_dataset/trivia_title_cont.tsv')
    ...
    if 'doc_aug' in args.query_type:
        if args.trivia:
            filename = join(args.data_dir, 'trivia_dataset/trivia_doc_aug.tsv')
            path_list.append(filename)
        elif args.nq:
...
def load_data_infer(args):
    ...
    if args.test_set == 'dev':
        if args.trivia:
            dev_file = join(args.data_dir, 'trivia_dataset/dev.tsv')
            test_file = join(args.data_dir, 'trivia_dataset/test.tsv')
            df = pd.read_csv(
                    dev_file,
                    encoding='utf-8', names=["query", "queryid", "oldid", "bert_k30_c30_1", "bert_k30_c30_2", "bert_k30_c30_3", "bert_k30_c30_4", "bert_k30_c30_5"],
